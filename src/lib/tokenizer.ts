...
export async function testTokenizerMetadata(modelId: string): Promise<{ success: boolean; message: string }> {
    const token = uiPreferences$.huggingfaceToken.get();
    const headers: Record<string, string> = {};
    if (token) {
        headers["Authorization"] = `Bearer ${token}`;
    ...
    try {
        const [jsonRes, configRes] = await Promise.all([
            fetch(`https://huggingface.co/${modelId}/resolve/main/tokenizer.json`, { method: 'HEAD', headers }),
            fetch(`https://huggingface.co/${modelId}/resolve/main/tokenizer_config.json`, { method: 'HEAD', headers })
        ]);

        if (jsonRes.ok && configRes.ok) {
            return { success: true, message: "Tokenizer metadata found!" };
        } else if (!jsonRes.ok && !configRes.ok) {
            return { success: false, message: "Tokenizer files not found for this model (unauthorized or missing)." };
        } else if (!jsonRes.ok) {
